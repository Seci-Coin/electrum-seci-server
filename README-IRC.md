IRC is used by Electrum Polis server to find 'peers' - other Electrum Polis servers. The
current list can be seen by running:

  electrum-polis-server peers

The following config file options are used by the IRC part of Electrum Polis server:
...
the Electrum Polis server will run in private mode (default). In private
mode, `electrum-polis-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum Polis
...
your Electrum Polis server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #electrum-polis .
