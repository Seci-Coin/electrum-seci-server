IRC is used by Electrum Seci server to find 'peers' - other Electrum Seci servers. The
current list can be seen by running:

  electrum-seci-server peers

The following config file options are used by the IRC part of Electrum Seci server:
...
the Electrum Seci server will run in private mode (default). In private
mode, `electrum-seci-server peers` will always return an empty list.

`host` is a fully-qualified domain name (FQDN) of your Electrum Seci
...
your Electrum Seci server instead of `host`. It is used as part of the name
...
composing the IRC nickname to identify your server on #electrum-seci .
