How to run your own Electrum Polis server
========================================
...
Electrum Polis server on Linux. It is structured as a series of steps you need to
...
The most up-to date version of this document is available at:
    ...
    https://github.com/polispay/electrum-polis-server/blob/master/HOWTO.md
...
document, we assume that user is called 'polis'). We also assume the
polis user has sudo rights, so we use `$ sudo command` when we need to.
...
you also need to run polisd and keep a copy of the full blockchain,
which is roughly 55 GB (February 2016). Ideally you have a machine with 16 GB of RAM
and an equal amount of swap. If you have ~2 GB of RAM make sure you limit polisd 
...
only leaving spare cycles for polisd. Fast single core CPU power helps for the initial 
...
### Step 1. Create a user for running polisd and Electrum Polis server

This step is optional, but for better security and resource separation I
suggest you create a separate user just for running `polisd` and Electrum.
We will also use the `~/bin` directory to keep locally installed files
(others might want to use `/usr/local/bin` instead). We will download source
code files to the `~/src` directory.

    $ sudo adduser polis --disabled-password
    $ sudo apt-get install git
    $ sudo su - polis
    $ mkdir ~/bin ~/src
    $ echo $PATH

If you don't see `/home/polis/bin` in the output, you should add this line
to your `.bashrc`, `.profile`, or `.bash_profile`, then logout and relogin:

    PATH="$HOME/bin:$PATH"
    $ exit

### Step 2. Download polisd

Recommend downloading latest version directly from Polis.org

    $ sudo apt-get install make g++ python-leveldb libboost-all-dev libssl-dev libdb++-dev pkg-config libevent-dev
    $ sudo su - polis
    $ cd ~/src && wget https://www.polis.org/binaries/polis-0.12.0.57-linux64.tar.gz
    $ sha256sum polis-0.12.0.57-linux64.tar.gz | grep 2a29b529c56d2ba41e28dfb20872861d6b48bdfe4fb327bfd2273123b38139aa
    $ tar xfz polis-0.12.0.57-linux64.tar.gz
    $ cd polis-0.12.0/bin
    $ cp -a polisd polis-cli polis-tx ~/bin

### Step 3. Configure and start polisd

In order to allow Electrum Polis to "talk" to `polisd`, we need to set up an RPC
username and password for `polisd`. We will then start `polisd` and
wait for it to complete downloading the blockchain.

    $ mkdir ~/.polis
    $ $EDITOR ~/.polis/polis.conf

Write this in `polis.conf`:
...
If you have an existing installation of polisd and have not previously
set txindex=1 you need to reindex the blockchain by running

    $ polisd -reindex

If you already have a freshly indexed copy of the blockchain with txindex start `polisd`:

    $ polisd

Allow some time to pass for `polisd` to connect to the network and start
downloading blocks. You can check its progress by running:

    $ polis-cli getblockchaininfo

Before starting the Electrum Polis server your polisd should have processed all
blocks and caught up to the current height of the network (not just the headers).
You should also set up your system to automatically start polisd at boot
time, running as the 'polis' user. Check your system documentation to
find out the best way to do this.

### Step 4. Download and install Electrum Polis server

We will download the latest git snapshot for Electrum to configure and install it:

    $ cd ~
    $ git clone https://github.com/polispay/electrum-polis-server.git
    $ cd electrum-polis-server
...
### Optional Step 5: Install Electrum Polis dependencies manually

Electrum Polis server depends on various standard Python libraries and leveldb. These will usually be
...
Electrum Polis server uses leveldb to store transactions. You can choose
...
and further instructions from the Electrum Polis full archival server foundry at a later time.
...
server.key corresponds to `ssl_keyfile=` in your Electrum Polis server config.
...
### Step 9. Configure Electrum Polis server

Electrum Polis reads a config file (/etc/electrum-polis.conf) when starting up. This
file includes the database setup, polisd RPC setup, and a few other
options.

The "configure" script listed above will create a config file at /etc/electrum-polis.conf
...
Electrum Polis server currently needs quite a few file handles to use leveldb. It also requires
file handles for each connection made to the server. It's good practice to increase the
open files limit to 64k.

The "configure" script will take care of this and ask you to create a user for running electrum-polis-server.
If you're using the user `polis` to run electrum and have added it as shown in this document, run
the following code to add the limits to your /etc/security/limits.conf:

     echo "polis hard nofile 65536" >> /etc/security/limits.conf
     echo "polis soft nofile 65536" >> /etc/security/limits.conf
...
Check if the limits are changed either by logging with the user configured to run Electrum Polis server as. Example:
...
Or if you use sudo and the user is added to sudoers group:
    ...
    sudo -u polis -i ulimit -n


Two more things for you to consider:

1. To increase privacy of transactions going through your server
   you may want to close polisd for incoming connections and connect outbound only. Most servers do run
   full nodes with open incoming connections though.

2. Consider restarting polisd (together with electrum-server) on a weekly basis to clear out unconfirmed
   transactions from the local the memory pool which did not propagate over the network.

### Step 11. (Finally!) Run Electrum Polis server

The magic moment has come: you can now start your Electrum Polis server as root (it will su to your unprivileged user):
...
You should see this in the log file:
    ...
    starting Electrum Polis server
...
If you want to stop Electrum Polis server, use the 'stop' command:
...
### Step 12. Test the Electrum Polis server
...
current server and connect to your new Electrum Polis server. You should see your
addresses and transactions history. You can see the number of blocks and
response time in the server selection window. You should send/receive some
poliss to confirm that everything is working properly.
...
If you're operating a public Electrum Polis server please subscribe
